...
function DiffText({ original, corrected }: { original: string; corrected: string }) {
  ...
  return (
    <p className="text-sm leading-relaxed">
      {result.map((t, idx) =>
        t.changed
          ? <mark key={idx} className="bg-emerald-500/30 text-emerald-300 rounded px-0.5 mx-0.5 font-semibold not-italic">{t.word}</mark>
...
export default function DialoguePage() {
  ...
  const fetchDueCount = useCallback(() => {
    fetch(`${API}/words/due`).then(r => r.json()).then((w: unknown[]) => setDueCount(w.length)).catch(() => {});
  }, []);
  ...
  const generateDialogue = useCallback(async (level?: CEFRLevel) => {
    ...
    try {
      ...
      alert("Error generating dialogue. Is the backend running?");
    } finally {
      setLoading(false);
    }
  }, [cefrLevel]);
  ...
  const startRevealing = () => {
    if (!dialogue || !Array.isArray(dialogue.lines) || dialogue.lines.length === 0 || isRevealing) return;
    setIsRevealing(true);
    setVisibleCount(0);
    let i = 0;
    const tick = () => {
  ...
  const handleSubmit = async () => {
    if (!dialogue || summary.trim().length < 10) return;
    setEvaluating(true);
    try {
      const res = await fetch(`${API}/dialogue/evaluate`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dialogue, summary, level: cefrLevel }),
      });
      const data: EvalResult = await res.json();
      setResult(data);
      if (data.xp > 0) {
        setSessionXP(s => s + data.xp);
        fetch(`${API}/xp/add`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      alert("Error evaluating. Check backend.");
    } finally {
      setEvaluating(false);
    }
  };

  const dialogueDone = dialogue && Array.isArray(dialogue.lines) && visibleCount >= dialogue.lines.length;
  const cfg = levelConfig[cefrLevel];

  // Avatar color per speaker index
  const avatarColors = ["bg-violet-600", "bg-cyan-600"];
    ...
    <main className="min-h-screen bg-[#0f0f1a] text-white flex flex-col items-center px-4 py-10">
      ...
      <div className="w-full max-w-2xl mb-8">
        <div className="flex items-center justify-between mb-1">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Read a conversation and summarize it in English.</p>
          </div>
          <div className="flex items-center gap-3">
            {sessionXP > 0 && <span className="text-violet-300 font-bold text-sm">+{sessionXP} XP</span>}
            <button
      ...
      <div className="w-full max-w-2xl mb-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
          <p className="text-xs text-gray-400 uppercase tracking-widest font-semibold mb-3">Dialogue difficulty</p>
          <div className="grid grid-cols-6 gap-2">
            {(Object.keys(levelConfig) as CEFRLevel[]).map((lvl) => {
              ...
              return (
                <button key={lvl}
                  onClick={() => { setCefrLevel(lvl); setDialogue(null); setResult(null); setSummary(""); }}
      ...
      {!dialogue && !loading && (
        <div className="w-full max-w-2xl mb-6">
          <button
            onClick={() => generateDialogue(cefrLevel)}
      ...
      {loading && (
        <div className="w-full max-w-2xl mb-6">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 flex items-center justify-center gap-3">
            ...
            <span className="text-gray-400 text-sm">Generating dialogue...</span>
          </div>
        </div>
      )}
        ...
        <div className="w-full max-w-2xl mb-6 animate-fade-in">
          <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="relative px-6 pt-5 pb-4 border-b border-white/10">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold capitalize">{dialogue.scenario}</p>
                    <p className="text-gray-400 text-xs mt-0.5">{dialogue.characters.join(" & ")}</p>
                  </div>
                </div>
            ...
            <div className="p-4 space-y-3 min-h-[200px]">
              {visibleCount === 0 && !isRevealing && (
                <div className="flex items-center justify-center py-10">
                  <button
                    onClick={startRevealing}
                    className="bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white font-semibold px-8 py-3 rounded-xl transition-all"
                  >
                    ▶ Start reading
                  </button>
                </div>
              ...
              {dialogue.lines.slice(0, visibleCount).map((line, idx) => {
                const speakerIdx = dialogue.characters.indexOf(line.speaker);
                const isRight = speakerIdx === 1;
                const avatarColor = avatarColors[speakerIdx] ?? "bg-gray-600";
                const initial = line.speaker.charAt(0).toUpperCase();

                return (
                  <div key={idx} className={`flex items-end gap-2 animate-fade-in ${isRight ? "flex-row-reverse" : ""}`}>
                    {/* Avatar */}
                    ...
                    <div className={`max-w-[75%] ${isRight ? "items-end" : "items-start"} flex flex-col gap-1`}>
                      <span className="text-[10px] text-gray-500 px-1">{line.speaker}</span>
                      <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                        ...
                        {line.text.split(/(\s+)/).map((token, ti) =>
                          /\s+/.test(token) ? (
            ...
            {isRevealing && (
              <div className="px-4 pb-4">
                <button
                  onClick={() => { setIsRevealing(false); setVisibleCount(dialogue.lines.length); }}
                  className="w-full text-xs text-gray-400 hover:text-white border border-white/10 hover:border-white/20 rounded-xl py-2 transition-all"
                >
                  Skip — show all
                </button>
              </div>
      ...
      {/* Summary section — shown after dialogue is fully revealed */}
      {dialogueDone && !isRevealing && (
        <div className="w-full max-w-2xl mb-6 animate-fade-in">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <label className="block text-sm font-semibold text-gray-300 mb-1">
              📝 What was this dialogue about?
            </label>
            <p className="text-gray-500 text-xs mb-3">Explain in English what happened in the conversation.</p>
            <textarea
              value={summary}
            ...
            <div className="flex items-center justify-between mt-3">
              <span className="text-xs text-gray-500">{summary.length} chars</span>
              {!result ? (
                <button
                  onClick={handleSubmit}
                  disabled={evaluating || summary.trim().length < 10}
          ...
          <div className={`rounded-2xl p-6 border ${
            ...
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right ml-4 shrink-0">
                <div className={`text-3xl font-black ${result.xp > 0 ? "text-violet-300" : "text-gray-500"}`}>+{result.xp}</div>
